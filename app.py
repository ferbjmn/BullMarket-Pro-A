...
st.set_page_config(
    page_title="📊 Dashboard Financiero Avanazado",
    layout="wide",
    page_icon="📈",
        ...
        with tab_facturacion:
            ...
            for _, empresa in df.iterrows():
                try:
                    ...
                    # Obtener balance sheet y financials
                    balance_sheet = tkr.balance_sheet
                    ...
                    # Buscar efectivo
                    cash = 0
                    cash_keys = ["Cash And Cash Equivalents", "Cash And Cash Equivalents At Carrying Value"]
                    for key in cash_keys:
                        if key in balance_sheet.index:
                            cash = balance_sheet.loc[key, latest_date]
                            break
                    ...
                    # Buscar facturación (revenue)
                    revenue = 0
                    revenue_keys = ["Total Revenue", "Revenue"]
                    for key in revenue_keys:
                        if key in financials.index:
                            revenue = financials.loc[key, latest_date]
                            break
                    ...
                    facturacion_metrics.append({
                        ...
                        "Deuda Total": total_debt,
                        "Deuda Neta": net_debt,
                        "Deuda/Facturación": debt_to_revenue,
                        "Deuda Neta/Facturación": net_debt_to_revenue,
            ...
            if facturacion_metrics:
                ...
                # Formatear números grandes
                display_df["Facturación Anual"] = display_df["Facturación Anual"].apply(lambda x: f"${x/1e6:,.0f}M" if pd.notnull(x) else "N/D")
                display_df["Deuda Total"] = display_df["Deuda Total"].apply(lambda x: f"${x/1e6:,.0f}M" if pd.notnull(x) else "N/D")
                ...
                valid_data = fact_df[fact_df["Deuda Neta/Facturación"] != float('inf')]
                ...
                colors_pie = ['green', 'lightgreen', 'orange', 'red'][:len(eval_counts)]
                ...
                if selected_ticker:
                    ...
                    with col3:
                        ...
                        elif empresa_data['Deuda Neta/Facturación'] < 0.15:
                            recommendation = "Buena posición. Deuda manejable."
                        elif empresa_data['Deuda Neta/Facturación'] < 0.4:
